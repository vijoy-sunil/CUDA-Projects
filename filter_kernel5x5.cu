...
__global__ void SobelFilter(const float* s_SobelMatrix,unsigned char* g_DataIn, unsigned char* g_DataOut, int width, int height)
{
...
//    s_SobelMatrix[8] = 1;
   ...
   if ((x > width - FILTER_RADIUS - 1)&&(x <width)) {
    ...
    if(x <= blockIdx.x * TILE_WIDTH + 1) //left
       return;

    if(x > blockIdx.x * TILE_WIDTH + TILE_WIDTH + 1) //right
       return;

    if(y <= blockIdx.y * TILE_HEIGHT + 1) //top
       return;

    if(y > blockIdx.y * TILE_HEIGHT + TILE_HEIGHT + 1)
       return; //bottom
