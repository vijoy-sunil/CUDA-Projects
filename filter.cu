...
char *BMPOutFile = "/home/students/visu3975/NVIDIA_GPU_Computing_SDK/C/src/Filter/lena_boost_filter.bmp";
//char *Filter = "sobel";
//char *Filter = "average";
char *Filter = "boost";
...
void FilterWrapper(unsigned char* pImageIn, int Width, int Height)
{
   ...
   switch(FilterMode) {
     ...
     cutilCheckError(cutStopTimer(timer_GPU));
     printf("AVERAGE FILTER GPU Execution Time: %f (ms) \n",cutGetTimerValue(timer_GPU));
     cutilCheckMsg("kernel launch failure");
     break;
     ...
     cutilCheckError(cutStopTimer(timer_GPU));
     printf("BOOST FILTER GPU Execution Time: %f (ms) \n",cutGetTimerValue(timer_GPU));
     cutilCheckMsg("kernel launch failure");
     break;
    }
